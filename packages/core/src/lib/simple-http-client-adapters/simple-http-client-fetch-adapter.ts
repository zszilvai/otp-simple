...
export class SimpleHttpClientFetchAdapter implements SimpleHttpClient {
  ...
  constructor(private fetch: (url: RequestInfo, init?: RequestInit | undefined) => Promise<Response>) {}

  async post(url: string, body: string, signature: string): Promise<SimpleCallResult> {
